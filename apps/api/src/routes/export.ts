...
import { NotFoundError, AuthError, ValidationError } from "../lib/errors";
import { getCall, getLatestAnalysis } from "../services/firestore";
import { isUserParticipant } from "../services/matcher";
import type { AnalysisDoc, CallDoc } from "../lib/types";
...
/**
 * GET /api/calls/:callId/export?format=md|pdf
 * Export call analysis as a report.
 */
exportRoutes.get("/calls/:callId/export", async (c) => {
  ...
  if (!analysis || analysis.status !== "completed") {
    throw new NotFoundError("Completed analysis");
  }

  if (format === "md" || format === "pdf") {
    // For v1, both formats return markdown. PDF would need a headless browser.
    const markdown = generateMarkdownReport(callDoc, analysis);
    return new Response(markdown, {
      headers: {
        "Content-Type": "text/markdown; charset=utf-8",
        "Content-Disposition": `attachment; filename="${callDoc.title} - Analysis.md"`,
      },
    });
  }
...
function generateMarkdownReport(
  ...
  lines.push(`# Call Analysis: ${call.title}`);
  lines.push("");
  lines.push(`**Date:** ${call.callDate}`);
  lines.push(`**Gong Call ID:** ${call.gongCallId}`);
  ...
  if (!result) {
    lines.push("---");
    lines.push("*Generated by GongIntel*");
    return lines.join("\n");
  }
